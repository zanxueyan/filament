...
size_t Transcoder::operator()(float* target, void const* source, size_t count) const noexcept {
    ...
    switch (mConfig.componentType) {
        case ComponentType::BYTE: {
            const int stride = mConfig.strideBytes ? mConfig.strideBytes : 1;
        ...
        case ComponentType::UBYTE: {
            int const stride = mConfig.strideBytes ? mConfig.strideBytes : 1;
            if (mConfig.normalized) {
        ...
        case ComponentType::SHORT: {
            const int stride = mConfig.strideBytes ? mConfig.strideBytes : 2;
            if (mConfig.normalized) {
        ...
        case ComponentType::HALF: {
            const int stride = mConfig.strideBytes ? mConfig.strideBytes : 2;
            //convert<filament::math::half>(target, source, count, comp, stride);
            return required;
        }
    }
    return 0;
