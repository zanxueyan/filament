...
//     } StructIdentifier ;
//
static std::string shrinkStructFields(const std::string& source) {

    // First split the string into codelines.
    ...
    enum {OUTSIDE, HEADER, BODY} state = OUTSIDE;
    ...
    for (std::string_view codeline : codelines) {
        ...
        switch (state) {
            case OUTSIDE: {
                ...
                if (index == codeline.size()) {
                    std::string dbg(typeId);
                    printf("bingo START: %s\n", dbg.c_str());
                    state = HEADER;
                }
                break;
            }
            case HEADER:
                state = (codeline[0] == '{' && codeline.size() == 1) ? BODY : OUTSIDE;
                break;
            case BODY:
                if (consumeString(codeline, &index, "} ")) {
                    ...
                    for (auto field : fields) {
                        const std::string key = std::string(structName) + "." + std::string(field);
                        const std::string val = std::string(structName) + "." + generatedFieldName;
                        // printf("%s => %s\n", key.c_str(), val.c_str());
...
bool GLSLPostProcessor::process(const std::string& inputShader, Config const& config,
    ...
    if (mGlslOutput) {
        ...
        *mGlslOutput = shrinkStructFields(*mGlslOutput);
        if (mPrintShaders) {
