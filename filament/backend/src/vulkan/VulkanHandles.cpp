...
VulkanRenderTarget::VulkanRenderTarget(VulkanContext& context, uint32_t width, uint32_t height,
    ...
    for (int index = 0; index < MRT::TARGET_COUNT; index++) {
        ...
        mColor[index].view = texture->getSidecarImageView(spec.level, spec.layer,
    ...
    if (depthTexture) {
        mDepth.view = depthTexture->getSidecarImageView(mDepth.level, mDepth.layer,
    ...
    for (int index = 0; index < MRT::TARGET_COUNT; index++) {
        ...
        if (texture && texture->samples == 1) {
            ...
            mMsaaAttachments[index].view = msTexture->getSidecarImageView(0, 0, VK_IMAGE_ASPECT_COLOR_BIT);
        }
        if (texture && texture->samples > 1) {
    ...
    mMsaaDepthAttachment.view = msTexture->getSidecarImageView(depthSpec.level, depthSpec.layer,
            VK_IMAGE_ASPECT_DEPTH_BIT);
}
...
VkImageView VulkanTexture::getSidecarImageView(int level, int layer, VkImageAspectFlags aspect) {
    ...
    mSidecarImageViews.emplace_back(ImageViewCacheEntry({level, layer, imageView}));
    ...
    assert(mSidecarImageViews.size() < 64);
